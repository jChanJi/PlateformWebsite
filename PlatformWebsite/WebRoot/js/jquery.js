...
(function(A, w) {
		...
		init : function(a, b) {
			...
			if (typeof a === "string")
				if ((d = Ta.exec(a)) &&
					...
					this.selector = a;
					this.context = s;
					a = s.getElementsByTagName(a);return c.merge(this,
						a)
	...
	c.extend = c.fn.extend = function() {
		...
		bindReady : function() {
			if (!xa) {
				...
				if (s.addEventListener) {
					...
					s.attachEvent("onreadystatechange", L);A.attachEvent("onload", c.ready);
					var a = false;
					try {
		...
		globalEval : function(a) {
			if (a && Va.test(a)) {
				...
				if (c.support.scriptEval) d.appendChild(s.createTextNode(a));else
					d.text = a;
		...
		uaMatch : function(a) {
			a = a.toLowerCase();
			a = /(webkit)[ \/]([\w.]+)/.exec(a) || /(opera)(?:.*version)?[ \/]([\w.]+)/.exec(a) || /(msie) ([\w.]+)/.exec(a) || !/compatible/.test(a) && /(mozilla)(?:.*? rv:([\w.]+))?/.exec(a) || [];return {
				browser : a[1] || "",
				version : a[2] || "0"
			}
		},
	...
	if (s.addEventListener)
		L = function() {
			s.removeEventListener("DOMContentLoaded", L, false);c.ready()
		};
	else if (s.attachEvent)
		L = function() {
			if (s.readyState === "complete") {
				s.detachEvent("onreadystatechange", L);c.ready()
			}
		};
	(function() {
		c.support = {};
		...
		if (!(!e || !e.length || !j)) {
			...
			a = s.createDocumentFragment();a.appendChild(d.firstChild);
			c.support.checkClone = a.cloneNode(true).cloneNode(true).lastChild.checked;c(function() {
				var k = s.createElement("div");
				k.style.width = k.style.paddingLeft = "1px";s.body.appendChild(k);
	...
	c.extend({
		...
		queue : function(a, b, d) {
			if (a) {
				...
				if (!d) return f || [];
				if (!f || c.isArray(d))
					f = c.data(a, b, c.makeArray(d));else f.push(d);
				return f
			}
		},
		...
		delay : function(a, b) {
			...
			b = b || "fx";return this.queue(b, function() {
				var d = this;
				setTimeout(function() {
					c.dequeue(d, b)
				}, a)
	...
	c.fn.extend({
		...
		attr : function(a, b, d, f) {
			...
			if (a.nodeType === 1) {
				...
				a = !c.support.hrefNormalized && f && j ? a.getAttribute(b, 2) : a.getAttribute(b);return a === null ? w : a
			}
	...
	c.event = {
			...
			if (!(a.nodeType === 3 || a.nodeType === 8)) {
				...
				if (j = c.data(a)) {
					...
					for (var k, n = 0, r; k = b[n++];) {
						...
						j.type = k;
						j.guid = d.guid;var u = i[k],
							z = c.event.special[k] || {};
						if (!u) {
							u = i[k] = [];
							if (!z.setup || z.setup.call(a, f, r, o) === false)
								if (a.addEventListener) a.addEventListener(k, o, false);else a.attachEvent && a.attachEvent("on" + k, o)
						}
						if (z.add) {
							z.add.call(a, j);
							if (!j.handler.guid)
		...
		trigger : function(a, b, d, f) {
			...
			if (!f) {
				...
				a.result = w;
				a.target = d;
				b = c.makeArray(b);b.unshift(a)
			}
			a.currentTarget = d;(f = c.data(d, "handle")) && f.apply(d, b);
			f = d.parentNode || d.ownerDocument;try {
	...
	s.addEventListener && c.each({
		...
		toggle : function(a) {
			...
			return this.click(c.proxy(a, function(f) {
				...
				c.data(this, "lastToggle" + a.guid, e + 1);f.preventDefault();return b[e].apply(this, arguments) || false
			}))
		},
		hover : function(a, b) {
	...
	c.each([ "live", "die" ], function(a, b) {
		...
		k.find = function(g, h, l) {
			...
			m || (m = h.getElementsByTagName("*"));return {
				set : m,
				expr : g
			}
		};
		k.filter = function(g, h, l, m) {
			for (var q = g, p = [], v = h, t, y, S = h && h[0] && x(h[0]); g && h.length;) {
				...
				if (g === q)
					if (y == null) k.error(g);else break;
				q = g
			}
			return v
		};
		...
		if (s.documentElement.compareDocumentPosition)
			...
			B = function(g, h) {
				...
				l.setStart(g, 0);l.setEnd(g, 0);m.setStart(h, 0);m.setEnd(h, 0);
	...
	var Ia = function(a, b, d) {
		if (c.isFunction(b)) return c.grep(a, function(e, j) {
			...
			if (Ua.test(b)) return c.filter(b, f, !d);else
				b = c.filter(b, f)
		}
	...
	c.fn.extend({
		...
		clone : function(a) {
			var b = this.map(function() {
				if (!c.support.noCloneEvent && !c.isXMLDoc(this)) {
					...
					if (!d) {
						d = f.createElement("div");d.appendChild(this.cloneNode(true));
						d = d.innerHTML
					}
		...
		domManip : function(a, b, d) {
			...
			if (c.isFunction(i)) return this.each(function(u) {
					var z = c(this);
					a[0] = i.call(this, u, b ? z.html() : w);z.domManip(a, b, d)
				});
			if (this[0]) {
				e = i && i.parentNode;
			...
			if (b === "width" || b === "height") {
				...
				function i() {
					e = b === "width" ? a.offsetWidth : a.offsetHeight;f !== "border" && c.each(j, function() {
						...
						if (f === "margin")
							e += parseFloat(c.curCSS(a, "margin" + this, true)) || 0;else
				...
				if (!j) {
					...
					C.onload = C.onreadystatechange = function() {
						if (!B && (!this.readyState || this.readyState === "loaded" || this.readyState === "complete")) {
							B = true;b();d();
							C.onload = C.onreadystatechange = null;z && C.parentNode && z.removeChild(C)
						}
					}
				}
	...
	c.fn.extend({
		...
		toggle : function(a, b) {
			...
			if (c.isFunction(a) && c.isFunction(b)) this._toggle.apply(this, arguments);else
		...
		animate : function(a, b, d, f) {
			...
			return this[e.queue === false ? "each" : "queue"](function() {
				...
				j.curAnim = c.extend({}, a);c.each(a, function(r, u) {
	...
	c.fx.prototype = {
		...
		show : function() {
			...
			this.options.show = true;this.custom(this.prop === "width" || this.prop === "height" ? 1 : 0, this.cur());c(this.elem).show()
		},
		hide : function() {
			this.options.orig[this.prop] = c.style(this.elem, this.prop);
		...
		step : {
			...
			_default : function(a) {
				if (a.elem.style && a.elem.style[a.prop] != null)
					a.elem.style[a.prop] = (a.prop === "width" || a.prop === "height" ? Math.max(0, a.now) : a.now) + a.unit;else
					a.elem[a.prop] = a.now
			}
		}
	});
	...
	c.offset = {
		initialize : function() {
			...
			this.doesNotIncludeMarginInBodyOffset = a.offsetTop !== j;a.removeChild(b);
		...
		position : function() {
			...
			f.left += parseFloat(c.curCSS(b[0], "borderLeftWidth", true)) || 0;return {
				top : d.top -
					f.top,
				left : d.left - f.left
			}
